...
isPalindromic n = reverseNum n == n

isLychrelAt :: Integer -> Integer -> Bool
isLychrelAt n m = not $ any isPalindromic nums
                  where
                    nums = [iter m i | i <- [1..n]]  -- not contain m.

-- help-function for isLychrelAt
iter :: Integer -> Integer -> Integer
iter n 0 = n
iter n x = iter (reverseNum n + n) (x-1)

main :: IO ()
main = print $ length $ filter (isLychrelAt 50) [1..(tenThousand-1)]